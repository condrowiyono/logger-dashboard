...
export const actions = {
  async get({commit, rootState}) {
    const { sortBy, descending, page, rowsPerPage } = rootState.facilities.pagination
    const { name } = rootState.facilities.params
    ...
    await this.$axios.get(`/facilities`, { params })
      .then((res) => {
        if (res.status === 200) {
  ...
  async show({commit}, params) {
    await this.$axios.get(`/facilities/${params.facility_id}`)
      .then((res) => {
        if (res.status === 200) {
  ...
  create({commit}, params) {
    return this.$axios.post(`/facilities`, params)
  },
  update({commit}, params) {
    return this.$axios.put(`/facilities/${params.id}`, params)
  },
  delete({commit}, params) {
    return this.$axios.delete(`/facilities/${params.id}`)
  }
};